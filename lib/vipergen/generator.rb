module Vipergen
  ...
  class Generator
    ...
    FILENAME_REPLACEMENT_KEY = "FILENAME"
    TARGET_REPLACEMENT_KEY = "TARGET"
    ...
    def self.generate_viper(template, language, name, path, target, author, company)
    ...
    def self.rename_file_content(filename, name, target, author, company)
      ...
      content = content.gsub((Vipergen::Generator::FILENAME_REPLACEMENT_KEY), File.basename(filename))
      content = content.gsub((Vipergen::Generator::TARGET_REPLACEMENT_KEY), target)
      content = content.gsub((Vipergen::Generator::AUTHOR_REPLACEMENT_KEY), author)
      content = content.gsub((Vipergen::Generator::DATE_REPLACEMENT_KEY), "#{Time.new.strftime("%m/%d/%y")}")
